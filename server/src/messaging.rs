...
impl MessageService {
    ...
    pub async fn send_admin_direct_message(&self, recipient_id: i64, content: &str) -> Result<i64, MessageError> {
        ...
        if let Some(tx) = clients.get(&recipient_id) {
            ...
            println!("[Admin] Client {} non connesso (salvato come non letto).", recipient_id);
        }
